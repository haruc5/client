...
function Detail(){
  ...
  const getDetail = async () => {
    try {
      const data = await(
        await fetch(`http://10.78.101.25:8082/api/challenge/detail/${id}`)
      ).json();
    ...
    <div className={styles.container}>
        <div>
          <div className={styles.banner}>
            <img
              src={challengeDetail.challengeImgUrl}
              alt=""
            />
          ...
          <div >
            ...
            {challengeInfo ? <ChallengeInfo
                      ...
                      : null}
            {challengeShot ? <ChallengeShot img={challengeDetail.challengeImgUrl} /> : null}
          </div>
        </div>
